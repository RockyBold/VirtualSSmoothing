...
class FABAttack():
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False, is_targeted=False):
        ...
        if is_targeted:
            ...
            if self.attack_vclasses:
                ...
                la_target = output[:, :self.num_in_classes].sort(dim=-1)[1][:, -self.target_class]
            la_target2 = la_target[pred].detach().clone()
            # print('la_target:{}, la_target2:{}'.format(la_target, la_target2))
    ...
    def perturb(self, x, y):
        ...
        adv = x.clone()
        with torch.no_grad():
            acc = self._predict_fn(x)[:, :self.num_in_classes].max(1)[1] == y
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        acc_curr = self._predict_fn(adv_curr)[:, :self.num_in_classes].max(1)[1] == y_to_fool
                        if self.norm == 'Linf':
                ...
                if self.attack_vclasses:
                    end_target_class = self.n_target_classes + self.start_target_class
                else:
                    end_target_class = min(self.n_target_classes + self.start_target_class, self.num_in_classes + 1)

                for target_class in range(self.start_target_class, end_target_class):
