...
class APGDAttack():
    ...
    def dlr_loss(self, x, y):
        if self.attack_vclasses:
            x_sorted, ind_sorted = x.sort(dim=1)
        else:
            x_sorted, ind_sorted = x[:, :self.num_in_classes].sort(dim=1)
    ...
    def dlr_corr_loss(self, x, y):
        if self.attack_vclasses:
            x_sorted, ind_sorted = x.sort(dim=1)
        else:
            x_sorted, ind_sorted = x[:, :self.num_in_classes].sort(dim=1)
        u = torch.arange(x.shape[0])
        return -x[u, y] / (x_sorted[:, -1] - x_sorted[:, -3] + 1e-12)

    def minimize_corr_loss(self, logits, y):
        u_idx = torch.arange(0, logits.size(0))
        return -logits[u_idx, y]

    def dlr_v_loss(self, x, y):
        x_sorted, ind_sorted = x.sort(dim=1)
        max_v = x[:, self.num_in_classes:].max(dim=1)[0]
        return -max_v / (x_sorted[:, -1] - x_sorted[:, -3] + 1e-12)

    def minimize_v_loss(self, logits, y):
        return -logits[:, self.num_in_classes:].max(dim=1)[0]
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            if self.loss == 'ce':
                ...
                criterion_indiv = lambda x, y: -1. * F.cross_entropy(x, y,
                    reduction='none')
            elif self.loss == 'minimize-corr':
                criterion_indiv = self.minimize_corr_loss
            elif self.loss == 'minimize-v':
                criterion_indiv = self.minimize_v_loss
            elif self.loss == 'dlr':
                criterion_indiv = self.dlr_loss
            elif self.loss == 'dlr-corr':
                criterion_indiv = self.dlr_corr_loss
            elif self.loss == 'dlr-v':
                criterion_indiv = self.dlr_v_loss
            elif self.loss == 'dlr-targeted':
                criterion_indiv = self.dlr_loss_targeted
            elif self.loss == 'dlr-n-corr-targeted':
                criterion_indiv = self.dlr_n_corr_loss_targeted
        ...
        acc = logits[:, :self.num_in_classes].detach().max(1)[1] == y
        acc_steps[0] = acc + 0
        ...
        for i in range(self.n_iter):
            ...
            pred = logits[:, :self.num_in_classes].detach().max(1)[1] == y
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        assert self.loss in ['ce', 'dlr', 'minimize-v', 'minimize-corr', 'dlr-corr', 'dlr-v'] #'ce-targeted-cfts'
        ...
        if not self.is_tf_model:
            y_pred = self.model(x)[:, :self.num_in_classes].max(1)[1]
        else:
            y_pred = self.model.predict(x)[:, :self.num_in_classes].max(1)[1]
        if y is None:
...
class APGDAttack_targeted(APGDAttack):
    def __init__(
            ...
            target_in_v_only=False,
            target_in_real_only=False
            ):
        """
        ...
        self.num_in_classes = num_in_classes
        self.start_target_class = 2
        self.attack_vclasses = attack_vclasses
        self.target_in_v_only = target_in_v_only
        if self.target_in_v_only:
            assert self.attack_vclasses == True
        self.target_in_real_only = target_in_real_only
        if self.target_in_real_only:
            assert self.target_in_v_only == False
    ...
    # with out minimizing the logit corresponding to the correct class
    def dlr_n_corr_loss_targeted(self, x, y):
        if self.attack_vclasses:
            x_sorted, ind_sorted = x.sort(dim=1)
        else:
            x_sorted, ind_sorted = x[:, :self.num_in_classes].sort(dim=1)
        ...
        return x[u, self.y_target] / (x_sorted[:, -1] - .5 * (x_sorted[:, -3] + x_sorted[:, -4]) + 1e-12)
    ...
    def perturb(self, x, y=None, x_init=None):
        ...
        assert self.loss in ['dlr-targeted', 'ce-targeted', 'dlr-n-corr-targeted']
        if not y is None and len(y.shape) == 0:
            x.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x)
        ...
        for target_class in range(self.start_target_class, self.n_target_classes + self.start_target_class):
            for counter in range(self.n_restarts):
                ind_to_fool = acc.nonzero().squeeze()
                if len(ind_to_fool.shape) == 0:
                    ind_to_fool = ind_to_fool.unsqueeze(0)
                if ind_to_fool.numel() != 0:
                    ...
                    if self.attack_vclasses:
                        if self.target_in_v_only:
                            assert self.n_target_classes <= output.size(1) - self.num_in_classes
                            self.y_target = output[:, self.num_in_classes:].sort(dim=1)[1][:,
                                            -target_class + 1] + self.num_in_classes
                        elif self.target_in_real_only:
                            self.y_target = output[:, :self.num_in_classes].sort(dim=1)[1][:, -target_class + 1]
                        else:
                            self.y_target = output.sort(dim=1)[1][:, -target_class]
                    else:
                        self.y_target = output[:, :self.num_in_classes].sort(dim=1)[1][:, -target_class]
                    # print('self.y_target:{}'.format(self.y_target[0:5]))
