...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        if self.attack_vclasses:
            ...
            y_others = logits[:, :self.num_in_classes].max(dim=-1)[0]
    ...
    def perturb(self, x, y=None):
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    ...
                    y_pred = output[:, :self.num_in_classes].max(1)[1]
                ...
                with torch.no_grad():
                    ...
                    if self.attack_vclasses:
                        ...
                        n_classes = output[:, :self.num_in_classes].shape[-1]
                    y_pred = output[:, :self.num_in_classes].max(1)[1]
        ...
        if not self.targeted:
            acc = self.predict(x)[:, :self.num_in_classes].max(1)[1] == y
        else:
            acc = self.predict(x)[:, :self.num_in_classes].max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                if not self.targeted:
                    acc_curr = output_curr[:, :self.num_in_classes].max(1)[1] == y_to_fool
                else:
                    acc_curr = output_curr[:, :self.num_in_classes].max(1)[1] != y_to_fool
