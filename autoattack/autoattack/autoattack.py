...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, num_in_classes=10, attack_vclasses=False):
        self.model = model
        self.norm = norm
        ...
        self.attack_vclasses = attack_vclasses
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=100, verbose=False, eps=self.epsilon,
                                   ...
                                   num_in_classes=num_in_classes, attack_vclasses=attack_vclasses)
            ...
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=100, eps=self.epsilon, seed=self.seed,
                                    ...
                                    attack_vclasses=attack_vclasses)
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=100, verbose=False, eps=self.epsilon,
                                   norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device,
                                   is_tf_model=True, num_in_classes=num_in_classes, attack_vclasses=attack_vclasses)
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                correct_batch = y.eq(output[:, :self.num_in_classes].max(dim=1)[1])
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    nat_pred_in_virtual_idcs = nat_whole_pred >= self.num_in_classes
                    ...
                    if attack == 'apgd-ce':
                    ...
                    elif attack == 'minimize-corr':
                        # apgd on dlr loss
                        self.apgd.loss = 'minimize-corr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y) #cheap=True

                    elif attack == 'minimize-v':
                        # apgd on dlr loss
                        self.apgd.loss = 'minimize-v'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y) #cheap=True
                    ...
                    elif attack == 'apgd-dlr-corr':
                        # apgd on dlr loss
                        self.apgd.loss = 'dlr-corr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y) #cheap=True

                    elif attack == 'apgd-dlr-v':
                        # apgd on dlr loss
                        self.apgd.loss = 'dlr-v'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y) #cheap=True
                        ...
                        self.apgd_targeted.loss = 'dlr-targeted'
                        adv_curr = self.apgd_targeted.perturb(x, y) #cheap=True

                    elif attack == 'apgd-ce-t':
                        # targeted apgd
                        self.apgd_targeted.seed = self.get_seed()
                        self.apgd_targeted.loss ='ce-targeted'
                        adv_curr = self.apgd_targeted.perturb(x, y) #cheap=True

                    elif attack == 'apgd-dlr-n-corr-t':
                        # targeted apgd
                        self.apgd_targeted.seed = self.get_seed()
                        self.apgd_targeted.loss ='dlr-n-corr-targeted'
                        adv_curr = self.apgd_targeted.perturb(x, y) #cheap=True
                    ...
                    _, pred = torch.max(output[:, :self.num_in_classes], dim=1)

                    _, adv_whole_pred = torch.max(output, dim=1)
                    adv_pred_in_virtual_idcs = adv_whole_pred >= self.num_in_classes
                    allowed_pred_in_virtual_idcs = torch.logical_and(nat_pred_in_virtual_idcs, adv_pred_in_virtual_idcs)
                    adv_pred_in_real_idcs = adv_whole_pred < self.num_in_classes
                    allowed_idcs = torch.logical_or(allowed_pred_in_virtual_idcs, adv_pred_in_real_idcs)
                    num_allowed_idcs = allowed_idcs.sum().item()
                    # print('num of adv_pred_in_virtual_idcs:', adv_pred_in_virtual_idcs.sum().item())
                    # print('num of adv_pred_in_real_idcs:', adv_pred_in_real_idcs.sum().item())
                    # print('num of allowed_idcs:', allowed_idcs.sum().item())

                    # discarded_idcs = torch.logical_and(~y.eq(pred), ~allowed_idcs)
                    # print('nat logits of discarded_idcs:{}'.format(nat_logits[discarded_idcs]))
                    # print('adv logits of discarded_idcs:{}'.format(output[discarded_idcs]))

                    # # y!= pred and adv_whole_pred are not newly located in vclasses
                    # false_batch = torch.logical_and(~y.eq(pred), allowed_idcs).to(robust_flags.device)

                    false_batch = ~y.eq(pred).to(robust_flags.device)
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
                    robust_flags[non_robust_lin_idcs] = False

                    x_adv[non_robust_lin_idcs] = adv_curr[false_batch].detach().to(x_adv.device)

                    num_misclassified_to_v += (x.shape[0] - num_allowed_idcs)
                    if self.verbose:
                        num_non_robust_batch = torch.sum(false_batch)
                        self.logger.log('{} - {}/{} - {} out of {} successfully perturbed ({} adv examples are misclassified to virtual classes)'
                                        .format(attack, batch_idx + 1, n_batches, num_non_robust_batch, x.shape[0], x.shape[0] - num_allowed_idcs))
                
                robust_accuracy = torch.sum(robust_flags).item() / x_orig.shape[0]
                if self.verbose:
                    self.logger.log(
                        'robust accuracy after {}: {:.2%} (number of adv example misclassified to virtual classes: {}, total time {:.1f} s)'.format(
                            attack.upper(), robust_accuracy, num_misclassified_to_v, time.time() - startt))
