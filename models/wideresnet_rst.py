...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, normalizer=None, output_real_only=False):
    ...
    def forward(self, x, return_prelogit=False):
        ...
        if return_prelogit:
            if self.output_real_only:
                return self.fc(out)[:, :self.num_real_classes], out[:, :self.num_real_classes]
            else:
                return self.fc(out), out
        else:
            if self.output_real_only:
                return self.fc(out)[:, :self.num_real_classes]
            else:
                return self.fc(out)
