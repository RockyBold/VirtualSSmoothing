...
def eval_from_data(model, x, y, batch_size, num_org_classes):
    model.eval()
    num_examples = len(x)
    correct = 0
    total = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--additional_nodes', default=10, type=int,
                        help='the number of additional pseudo label in output layer')
    ...
    if args.arch == 'WideResNet34':
        net = WideResNet(depth=34, num_classes=num_classes + args.additional_nodes, widen_factor=10)
    elif args.arch == 'WideResNet28':
        net = WideResNet(depth=28, num_classes=num_classes + args.additional_nodes, widen_factor=10)
    elif args.arch == 'PreActResNet18':
        net = PreActResNet18(num_classes=num_classes + args.additional_nodes)
    else:
    ...
    if not args.individual:
        adv_complete = adversary.run_standard_evaluation(x_test[:args.n_ex], y_test[:args.n_ex],
                                                         bs=args.batch_size)
        robust_acc, vnodes = eval_from_data(model, adv_complete, y_test, args.batch_size, num_classes)
        print('robust acc:', robust_acc, 'mis-classified classes:', vnodes)
